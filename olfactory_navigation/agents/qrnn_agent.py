...
class QRNNAgent(Agent):
    def __init__(self, 
        ...
        super().__init__(environment, threshold, "QRNNAgent")
    ...
    def choose_action(self) -> np.ndarray:
        ...
        eps_mask = (self.rnd_state.rand(self.current_state.shape[0]) < self.eps_greedy(self.episode) ).astype(bool)    
        if np.any(eps_mask):
    ...
    def train(self):#, init_sampling_region):
        cumulative_rewards = []
        average_crewards = []
        avg_max_void_steps = []
        max_void_steps = []
       ...
       # sampling_region = init_sampling_region.copy()
        for _ in iterator:
            # Initialization
            ...
            for t in range(self.horizon):
                ...
                if s == 0:
                    ...
                    a = self.rnd_state.choice(self.action_set.shape[0]) if self.rnd_state.rand() <= self.eps_greedy(self.episode) else np.argmax(self.Q[s, :])
