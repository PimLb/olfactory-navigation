...
# Test function
def run_test(grid, sub_grid, folder):
    # Variables
...
def main():
    ...
    root_folder = './7_test_results'
    os.mkdir(root_folder)

    for grid_s in grid_sizes:
        for sub_grid_s in sub_grid_sizes:
                grid = np.array([grid_s,grid_s])
                sub_grid = np.array([sub_grid_s,sub_grid_s])

                print('----------------------------')
                print(f'({grid_s} by {grid_s})' + ' - ' + f'({sub_grid_s} by {sub_grid_s})')

                folder = root_folder + '/' + f'{grid_s}_{grid_s}' + '-' + f'{sub_grid_s}_{sub_grid_s}'
                os.mkdir(folder)
