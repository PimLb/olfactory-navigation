...
for i in range(itStart, numberEpisodes):
    ...
    while( not isEnd(curState) and curStep < maxStepsPerEpisode):
        ...
        # Il grad vanilla dovrebbe essere phi(o,a) - sum_b pi(b|o)phi(o,b)
        # nel nostro caso phi è l'indicatrice, quindi dovrebbe essere così
        gradLogPi = np.zeros_like(theta)
        gradLogPi[obs, 0, action] = 1
        gradLogPi[obs, 0] -= pi[obs, 0] # Lo zero è per la memoria
        zActor = gamma * lambda_actor * zActor + discount * gradLogPi
